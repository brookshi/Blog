...
假设一个画图场景，里面有很多图元，如圆，矩形等，由于量非常多，且经常刷新，每次刷新new出一批图元，浪费内存是小事，临时小对象太多导致GC频繁运行才是麻烦事，UI会表现出卡顿。
...
```
下面建个图元工厂来创建、维护图元，工厂里维护了一个列表，包括圆和矩形，工厂除了创建图元外，还可以删掉所有图元，得到图元的问题（可见或不可见的）
```ts
class GraphFactory{
